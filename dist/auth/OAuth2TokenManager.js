  ...
  async refreshToken() {
    try {
      ...
      if (this.config.isLwa()) {
        ...
        response = await superagent.post(this.config.getCognitoEndpoint()).set('Content-Type', 'application/json').send({
        ...
        response = await superagent.post(this.config.getCognitoEndpoint()).set('Content-Type', 'application/x-www-form-urlencoded').send(requestBody);
      }
